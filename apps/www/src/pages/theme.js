...
export const getServerSideProps = async (context) => {
	...
	let warningMessage = mdOverrides?.warnings?.themes?.[json?.data?.id];
	let errorMessage = mdOverrides?.errors?.themes?.[json?.data?.id];

	const message = [warningMessage ? `> [!WARNING]\n> ${warningMessage}\n\n` : '', errorMessage ? `> [!CAUTION]\n> ${errorMessage}\n\n` : ''].filter(Boolean).join('\n\n');

	const readme = json?.read_me;
	const markdown = await MarkdownToHtml(readme, json?.data?.github?.owner, json?.data?.github?.repo, json?.commit_data?.oid, message);
...
export default function Home({ json, markdown, isSteamClient, isLinux, mdOverrides }) {
